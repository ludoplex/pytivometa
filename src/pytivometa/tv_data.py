...
def format_episode_data(ep_data, meta_filepath):
    ...
    for (pytivo_tag, ep_data_key) in pytivo_metadata:
        ...
        if ep_data.get(ep_data_key, ""):
            ...
            if (
                pytivo_tag == "episodeNumber"
                and ep_data["airedEpisodeNumber"]
                and int(ep_data["airedSeason"])
            ):
                text = "%d%02d" % (
                    int(ep_data["airedSeason"]),
                    int(ep_data["airedEpisodeNumber"]),
                )
...
class TvData:
    ...
    def search_tvdb_series_id(self, show_name):
        ...
        if year and len(series) > 1:
            LOGGER.debug(
                "2,There are %d matching series, but we know what "
    ...
    def search_rpc_series_id(self, show_name, first_aired="", actors=None):
        ...
        for series in results:
            LOGGER.debug(
                "2,%s - %s\n", series.get("title", ""), series.get("description", "")
            )
            LOGGER.debug("2,    first aired: %s", series.get("firstAired", ""))
            ...
            if tivo_program_id is not None:
                ...
                print(
                    "No RPC results for: {0} S{1:02}E{2:02}".format(
                        tv_info["series"], tv_info["season"], tv_info["episode"]
                    )
                )
